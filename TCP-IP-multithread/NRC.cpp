...
NRC::NRC(const string server_name, const string port) // client side constructor
{
	struct addrinfo hints, *res;
	//***********NOTE THAT SOCKFD IS THIS->SOCKFD
...
NRC::NRC(const string port, void (*handle_process_loop)(NRC *)) // server side constructor
{
    ...
    if (bind(sockfd, serv->ai_addr, serv->ai_addrlen) == -1) { //::bind ?
		close(sockfd);
		perror("server: bind");
		exit(0);
	}
	...
	{  // main accept() loop
        ...
        //pthread_create (handle_process_loop, sc);
