...
void worker_function(BoundedBuffer *BB, NRC *workChan, HistogramCollection *hist, string fARG)
{
    ///// a lot of shit needs to be deleted
...
int main(int argc, char *argv[])
{
    ...
    string port = "49152";
    string chan_name = "default";

    int opt;
    bool activeFile = false;

    while ((opt = getopt(argc, argv, "n:w:b:p:f:h:r:")) != -1)
        switch (opt)
        {
    ...
    */
    ...
    if (!activeFile)
    {
        ...
        for (int i = 0; i < w; i++)
        {
            NRC *workerChan = new NRC(chan_name, port); //where is chan_name coming from
    ...
    {
        NRC *chan = new NRC(chan_name, port); // I use chan_name and port here?
        ...
        for (int i = 0; i < w; i++)
        {
            NRC *workerChan = new NRC(chan_name, port); //is it right to put port here?
